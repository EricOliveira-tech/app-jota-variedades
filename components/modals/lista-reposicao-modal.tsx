...
import { getLowStockProducts } from "@/lib/mock-data";
...
export function ListaReposicaoModal({
  open,
  onClose,
}: ListaReposicaoModalProps) {
  ...
  const handleCopy = () => {
    ...
    const text = unchecked
      .map((p) => `• ${p.nome} (atual: ${p.estoque}, mínimo: ${p.minimo})`)
      .join("\n");

    navigator.clipboard.writeText(
      `Lista de Reposição - J Variedades\n\n${text}`,
    );
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="flex flex-col gap-4">
          {produtosBaixos.length === 0 ? (
            ...
            <>
              <p className="text-sm text-muted-foreground">
                {produtosBaixos.length} produtos precisam de reposição. Marque
                os itens já providenciados.
              </p>
