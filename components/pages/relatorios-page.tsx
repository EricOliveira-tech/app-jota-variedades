...
import { TrendingUp, Smartphone, Banknote, CreditCard, Package, ShoppingCart, ClipboardList } from "lucide-react"
import { getResumoVendas, vendas, getProdutosBaixoEstoque } from "@/lib/mock-data"
...
export function RelatoriosPage() {
  ...
  const produtosVendidos = vendasHoje.reduce(
    (acc, venda) => {
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex flex-col gap-4 p-4">
        ...
        {/* Payment Split */}
        <div>
          <h2 className="mb-3 font-semibold text-foreground">Por Forma de Pagamento</h2>
          <div className="grid grid-cols-3 gap-3">
            <Card className="bg-teal-50">
              <CardContent className="p-3 text-center">
                ...
                <p className="mt-1 text-sm font-semibold text-teal-700">
                  R$ {resumo.totalPix.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                </p>
              </CardContent>
            </Card>
            <Card className="bg-emerald-50">
              <CardContent className="p-3 text-center">
                ...
                <p className="text-2xl font-bold text-emerald-700">{resumo.dinheiro}</p>
                <p className="text-xs text-emerald-600">vendas</p>
                <p className="mt-1 text-sm font-semibold text-emerald-700">
                  R$ {resumo.totalDinheiro.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                </p>
              </CardContent>
            </Card>
            <Card className="bg-blue-50">
              <CardContent className="p-3 text-center">
                ...
                <p className="text-2xl font-bold text-blue-700">{resumo.cartao}</p>
                <p className="text-xs text-blue-600">vendas</p>
                <p className="mt-1 text-sm font-semibold text-blue-700">
                  R$ {resumo.totalCartao.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                </p>
              </CardContent>
            </Card>
          </div>
        ...
        {/* Top Products */}
        <div>
          <h2 className="mb-3 font-semibold text-foreground">Produtos Mais Vendidos</h2>
          {topProdutos.length === 0 ? (
            ...
            <Card>
              <CardContent className="divide-y divide-border p-0">
                {topProdutos.map(([nome, quantidade], index) => (
                  <div key={nome} className="flex items-center justify-between p-3">
                    ...
                    <span className="rounded-full bg-muted px-3 py-1 text-sm font-semibold">{quantidade} un.</span>
                  </div>
                ))}
              </CardContent>
            </Card>
          ...
          <Card className="border-amber-200 bg-amber-50">
            <CardContent className="p-4">
              <div className="flex items-center gap-2">
                ...
                <p className="font-semibold text-amber-800">{produtosBaixos.length} produtos precisam reposição</p>
              </div>
              <p className="mt-1 text-sm text-amber-600">
                {produtosBaixos.filter((p) => p.estoque === 0).length} em falta total
              </p>
            </CardContent>
          </Card>
        )}
      ...
      <ListaReposicaoModal open={showReposicao} onClose={() => setShowReposicao(false)} />
    </div>
  )
}
